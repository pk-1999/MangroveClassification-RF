...
Map.addLayer(roi, {color: 'red', opacity: 0.2},'region',false);
var startYear = ee.Number(2019);
var endYear = ee.Number(2023);

//var name_image = ee.List(['23-1','23-2']);
var name_image = ee.List(['19-1','19-2','20-1','20-2','21-1','21-2','22-1','22-2','23-1','23-2']);
var count = 2;
var folder = 'image_ZhanJiang';
var assetId = 'S2_and_L8-ZhanJiang';
...
function renameImage(image){
  return image.rename(['blue', 'green', 'red', 'nir']);
}
...
function refChart(processedImageCollection){
  var refMeanList = processedImageCollection.aggregate_array('ref_mean');
  var chart =  ui.Chart.array.values(refMeanList, 0)
  .setChartType('ColumnChart')
  .setOptions({
    title: 'reference mean - index',
    hAxis: { title: 'index' },
    vAxis: { title: 'ref_mean' },
    colors: ['red']
  });
  return chart;
}
...
var Landsat8 = L8.filter(ee.Filter.lt('CLOUD_COVER', 20))
                ...
                .map(renameImage)
                .map(addRefMean)
                .filter(ee.Filter.lt("ref_mean", 0.20))
                .filter(ee.Filter.gt("ref_mean", 0))
;
/*
...
// cloud free check
// 生成ref-mean条形图
//var chart_L9 = refChart(Landsat9);
//print('Landsat9 - reference mean 的条形图', chart_L9);
var chart_L8 = refChart(Landsat8);
print('Landsat8 - reference mean 的条形图', chart_L8);
//var chart_L7 = refChart(Landsat7);
//print('Landsat7 - reference mean 的条形图', chart_L7);


// 检查效果
//var error_L8 = ee.Image(Landsat8.toList(Landsat8.size()).get(8));
//print('ref_mean value for selected image:', error_L8.get('ref_mean'));
//Map.addLayer(error_L8, {band:['blue', 'green', 'red'], min:0, max:0.3}, 'error_L8');


//var chart_S2 = refChart(Sentinel2);
//print('Sentinel2 - reference mean 的条形图',chart_S2);


/*
// *********
// 时间序列影像合成为Image
function imageListPerSemiyear (startYear, endYear, processedImageCollection){
  var yearList = ee.List.sequence(startYear, endYear, 1);
  var dateList = ee.List([[[1,1],[6,30]],[[7,1],[12,31]]]);
  var processedImageList = yearList.map(function(year){
    year = ee.Number(year);
    return dateList.map(function(days){
      ...
      var images = processedImageCollection
...
// landsat
var L9_imageList = imageListPerSemiyear(startYear, endYear,Landsat9);
var L8_imageList = imageListPerSemiyear(startYear, endYear,Landsat8);
var L7_imageList = imageListPerSemiyear(startYear, endYear,Landsat7);
//print("L9_imageList:", L9_imageList);

// S2
var S2_imageList = imageListPerSemiyear(startYear, endYear,Sentinel2);
//print("S2_imageList:", S2_imageList);


for (var i = 0; i < count; i++)
{
  var S2 = ee.Image(S2_imageList.get(i));
  var L9 = ee.Image(L9_imageList.get(i));
  var L8 = ee.Image(L8_imageList.get(i));
  var L7 = ee.Image(L7_imageList.get(i));
  ...
  Map.addLayer(S2, visParams, 'S2_' + f_name, false);
  Map.addLayer(L9, visParams, 'L9_' + f_name, false);
  Map.addLayer(L8, visParams, 'L8_' + f_name, false);
  Map.addLayer(L7, visParams, 'L7_' + f_name, false);
}
